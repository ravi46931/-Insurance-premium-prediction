...
with col31:
    region = st.selectbox('Region:', ['Select Region','southwest', 'southeast', 'northwest', 'northeast'])
...
if st.button('Submit Text'):
        ...
        if (bmi_flag==False) or (gender_flag==False) or (smoker_flag==False) or (region_flag==False):
            if (bmi_flag==False) and ((gender_flag==False) or (smoker_flag==False) or (region_flag==False)):
            ...
            for i in range(len(inputs)):
                ...
                with col_name:
                    st.markdown(colored_text(input_names[i], 'white'), unsafe_allow_html=True)
                with col_input:
