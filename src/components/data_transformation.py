...
class DataTransformation:
    ...
    def transform_data(self, df):
        try:
            ...
            train_transformed_data = preprocessor.fit_transform(X_df[['region', 'sex', 'smoker', 'age', 'bmi']])
           
            # Get the feature names after transformation
            feature_names = get_feature_names(preprocessor, X_df[['region', 'sex', 'smoker', 'age', 'bmi']].columns)
           
            # Convert the transformed data to a DataFrame
            train_transformed_df = pd.DataFrame(train_transformed_data, columns=feature_names)
            train_df = pd.concat([train_transformed_df, X_df[['children']], Y_df[['expenses']]], axis=1)

            test_transformed_data = preprocessor.transform(X_test[['region', 'sex', 'smoker', 'age', 'bmi']])
            test_transformed_df = pd.DataFrame(test_transformed_data, columns=feature_names)
            test_df = pd.concat([test_transformed_df, X_test[['children']], y_test[['expenses']]], axis=1)
    ...
    def initiate_data_transformation(self):
        try:
            ...
            joblib.dump(preprocessor, self.data_transformation_config.PREPROCESSOR_FILE_PATH)
