...
class ModelEvaluation:
    ...
    def model_evaluation(self,model_names, models):
        try:
            ...
            for i in range(len(model_names)):
                ...
                r2_score_values[model_names[i]] = round(r2_score(y, model.predict(X)), 2)

            metrics = {
               'MAE': mae,
               'MSE': mse,
               'R2_SCORE': r2_score_values
                }
    ...
    def initiate_model_evaluation(self):
        try:
            df = pd.read_csv(self.data_transformation_artifact.train_transform_file_path)
            X = df.drop(columns=['expenses', 'Unnamed: 0'], axis=1)
            y = df[['expenses']].squeeze()

            with open(self.model_trainer_artifact.top_models_file_name_path, 'r') as json_file:
            ...
            with open(self.model_evaluation_config.TRAIN_METRICS_FILE_PATH, 'w') as json_file:
                json.dump(metrics, json_file, indent=4)
            
            best_model, best_model_name, test_metrics, best_model_metrics_test  = self.model_evaluation(model_names, models)
            ...
            with open(self.model_evaluation_config.TEST_METRICS_FILE_PATH, 'w') as json_file:
                json.dump(test_metrics, json_file, indent=4)
            
            # Saving the best model metrics
            with open(self.model_evaluation_config.BEST_MODEL_METRICS_FILE_PATH, 'w') as json_file:
