    ...
    def best_models(self, X, y, models):
        try:
            ...
            for key, model in models.items():
                ...
                print(f"{key} regression on training set MAE: {mean_absolute_error(y, model.predict(X)):.2f}")
    ...
    def initiate_model_trainer(self):
        try:
            ...
            for model_name,_ in top_models.items():
                ...
                file_path = os.path.join(self.model_trainer_config.MODELS_DIR_PATH, file_name)
                with open(file_path, 'wb') as f:
                    pickle.dump(models[model_name], f)
                file_paths.append(file_path)
                model_names.append(model_name)
            with open(self.model_trainer_config.TOP_MODELS_NAME_FILE_PATH, 'w') as json_file:
                json.dump(model_names, json_file, indent=4)
            
            model_trainer_artifact = ModelTrainerArtifact(
                self.model_trainer_config.TOP_MODELS_NAME_FILE_PATH,
                *file_paths
            )
